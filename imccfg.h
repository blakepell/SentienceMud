...
#define CH_IMCRANK(ch)           (IS_IMMORTAL(ch) ? "Staff" : sub_class_table[ch->pcdata->sub_class_current].name[ch->sex])
#endif
...
#define CH_IMCLEVEL(ch)          ((ch->tot_level))
#define CH_IMCNAME(ch)           GET_NAME(ch)
#define CH_IMCTITLE(ch)          GET_TITLE(ch)
#define CH_IMCRANK(ch)           (GET_SEX(ch) == SEX_FEMALE ? title_female(GET_CLASS(ch), (ch->tot_level))	\
								: title_male(GET_CLASS(ch), (ch->tot_level)))
