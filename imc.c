...
const char *imc_send_social( CHAR_DATA * ch, char *argument, int telloption );
void imc_save_config( void );
void imc_save_channels( void );
static void fix_separator_string(char* string);
static char* fix_sender(char* string);
SERVER_DATA *first_server;
SERVER_DATA *last_server;
...
char *imc_mudof_delim( char *src, char delim)
{
   static char mud[SMST];
   char *person;

   if( !( person = strchr( src, delim ) ) )
      imcstrlcpy( mud, src, SMST );
   else
      imcstrlcpy( mud, person + 1, SMST );
   return mud;
...
/*
  Hack to make the *dalet server show up properly. mtfox 10-12-08
*/
   if( !strcasecmp( p->name, "*dalet" ) )
   p->network = IMCSTRALLOC( "LPMuds.net" );
...
bool check_mudof_delim( CHAR_DATA * ch, char *mud, char delim)
{
   return check_mud( ch, imc_mudof_delim( mud, delim ) );
}
...
/*
Look for local name first so we return to the right server
*/
...
/* if we did not find it in that loop, lets look for cname */
   for( c = first_imc_channel; c; c = c->next )
     if( ( c->name && !strcasecmp( c->name, name ) ) )
          return c;
...
void imcformat_channel( CHAR_DATA * ch, IMC_CHANNEL * d, int format, bool all )
{
   ...
   if( all )
   {
      for( c = first_imc_channel; c; c = c->next )
      {
         ...
         {
            snprintf( buf, LGST, "~B[~D%s~B] ~W%%s: ~w%%s~!", c->local_name );
            IMCSTRFREE( c->regformat );
            c->regformat = IMCSTRALLOC( buf );
         }
         if( format == 2 || format == 4 )
         {
            snprintf( buf, LGST, "~B[~D%s~B] ~w%%s %%s~!", c->local_name );
            IMCSTRFREE( c->emoteformat );
            c->emoteformat = IMCSTRALLOC( buf );
         }
         if( format == 3 || format == 4 )
         {
            snprintf( buf, LGST, "~B[~D%s~B] ~w%%s~!", c->local_name );
            IMCSTRFREE( c->socformat );
   ...
   else
   {
      ...
      if( format == 1 || format == 4 )
      {
         snprintf( buf, LGST, "~B[~D%s~B] ~W%%s: ~w%%s~!", d->local_name );
         IMCSTRFREE( d->regformat );
         d->regformat = IMCSTRALLOC( buf );
      }
      if( format == 2 || format == 4 )
      {
         snprintf( buf, LGST, "~B[~D%s~B] ~w%%s %%s~!", d->local_name );
         IMCSTRFREE( d->emoteformat );
         d->emoteformat = IMCSTRALLOC( buf );
      }
      if( format == 3 || format == 4 )
      {
         snprintf( buf, LGST, "~B[~D%s~B] ~w%%s~!", d->local_name );
...
void imc_write_buffer( const char *txt, SERVER_DATA *server )
{
    ...
    */
   while( server->outtop + length >= server->outsize )
   {
      ...
      server->outsize *= 2;
      IMCRECREATE( server->outbuf, char, server->outsize );
   }

   /*
    * Copy.
    */
   strncpy( server->outbuf + server->outtop, output, length );   /* Leave this one alone! BAD THINGS(TM) will happen if you don't! */
   server->outtop += length;
   server->outbuf[server->outtop] = '\0';
}

SERVER_DATA *imc_find_network( IMC_PACKET * p )
{
   SERVER_DATA *server;

   for( server = first_server; server; server = server->next )
   {
      if( server && p->network != NULL && server->network && !strcasecmp( server->network, p->network ) && server->state == IMC_ONLINE )
         return server;
   }
   return NULL;
}
...
server = imc_find_network( p );
   ...
   if ( server )
    imc_write_buffer( txt, server );
   else
    imclog("Network not found for packet containing: %s", txt);
}
...
IMC_PACKET *imc_newpacket( const char *from, const char *type, const char *to, const char *network )
{
   IMC_PACKET *p;
   ...
   {
      imcbug( "%s: Attempt to build %s packet with no network field.", __FUNCTION__, type );
      return NULL;
   }
   ...
   imcstrlcpy( p->network, network, SMST );
   p->first_data = p->last_data = NULL;

   return p;
}

char *imc_network_of( const char *name)
	{
    REMOTEINFO *p;
   ...
   for( p = first_rinfo; p; p = p->next )
   {
      ...
      /* NULL is bad here, lets return something else */
	return ("Unknown");
	}
...
void imc_send_tell( const char *from, const char *to, const char *txt, int reply )
{
   ...
   p = imc_newpacket( from, "tell", to , imc_network_of( imc_mudof(to) ) );
...
static char* fix_sender(char* string)
{
	...
	*z++ = '\0';
	...
	sprintf(buf, "%s%c%s", senders, this_imcmud->channsep, from);
	return buf;
}

char* fix_sending(char* string)
{
	char from[SMST];
	char senders[SMST];
	static char buf[SMST];
	...
	*z++ = '\0';
	...
	sprintf(buf, "%s%s%s", senders, "@", from);
	return buf;
}
...
PFUN( imc_recv_chanwho )
{
   ...
   p = imc_newpacket( "*", "ice-chan-whoreply", q->from , imc_network_of( imc_mudof(q->from) ));
   imc_addtopacket( p, "channel=%s", c->name );
...
void imc_send_whoreply( const char *to, const char *txt )
{
   ...
   p = imc_newpacket( "*", "who-reply", to , imc_network_of( imc_mudof(to) ));
   imc_addtopacket( p, "text=%s", txt );
   imc_write_packet( p );
}
...
void imc_send_who( const char *from, const char *to, const char *type )
{
   ...
   p = imc_newpacket( from, "who", to , imc_network_of( imc_mudof(to) ));
...
/* Finger code */
void imc_process_finger( const char *from, char *type )
{
   CHAR_DATA *victim;
...
void imc_send_whoisreply( char *to, char *data )
{
   ...
   p = imc_newpacket( "*", "whois-reply", to , imc_network_of( imc_mudof(to) ));
...
void imc_send_whois( const char *from, const char *user )
{
   ...
   p = imc_newpacket( from, "whois", user , imc_network_of( imc_mudof(user) ));
   imc_write_packet( p );
}
...
void imc_send_beep( const char *from, const char *to )
{
   ...
   p = imc_newpacket( from, "beep", to , imc_network_of( imc_mudof(to) ));
   imc_write_packet( p );
}
...
PFUN( imc_send_keepalive )
{
   ...
   if( q ) {
      p = imc_newpacket( "*", "is-alive", q->from , imc_network_of( imc_mudof(q->from) ));
   ...
   SERVER_DATA *server;
        for( server = first_server; server; server = server->next ) {
			if (server && server->state == IMC_ONLINE)
			{
      p = imc_newpacket( "*", "is-alive", packet , server->network);
   imc_addtopacket( p, "versionid=%s", this_imcmud->versionid );
   imc_addtopacket( p, "url=%s", this_imcmud->www );
   imc_addtopacket( p, "host=%s", this_imcmud->ihost );
   imc_addtopacket( p, "port=%d", this_imcmud->iport );
...
void imc_request_keepalive( SERVER_DATA *server )
{
   IMC_PACKET *p;

   p = imc_newpacket( "*", "keepalive-request", "*@*" , server->network);
   imc_write_packet( p );

   imc_send_keepalive( NULL, "*@*" );
}

void imc_firstrefresh( SERVER_DATA *server )
{
   IMC_PACKET *p;

   p = imc_newpacket( "*", "ice-refresh", "IMC@$", server->network);
   imc_write_packet( p );
}
...
void imc_send_ucache_update( const char *visname, int gender)
{
   ...
   for( server = first_server; server; server = server->next )
  {
   ...
   p = imc_newpacket( visname, "user-cache", "*@*" , server->network);
...
void imc_send_ucache_request( char *targetuser )
{
   ...
   p = imc_newpacket( "*", "user-cache-request", to , imc_network_of( imc_mudof(to) ));
...
PFUN( imc_recv_ucache_request )
{
   ...
   p = imc_newpacket( "*", "user-cache-reply", to , imc_network_of( imc_mudof(to) ));
...
void imc_parse_packet( char *packet )
{
    ...
    */
	 ...
	 char *start, *onpath;
	 char serverpath[LGST];
       int  end;

      if( p->route && p->route[0] != '\0' )
      {
         if( ( start = strchr( p->route, '!' ) ) != NULL )
         {
            start++;
            onpath = start;
            end = 0;
...
void imc_finalize_connection( char *name, char *netname, SERVER_DATA *server)
{
   char *network_name;

   server->state = IMC_ONLINE;

   IMCSTRFREE( server->servername );
   server->servername = IMCSTRALLOC( name );

   network_name = IMCSTRALLOC( ( server->network && server->network[0] != '\0' ) ? server->network : "Unknown" );

   server->network = IMCSTRALLOC( ( netname && netname[0] != '\0' ) ? netname : network_name);
   imclog( "Connected to %s. Network ID: %s", name, ( netname && netname[0] != '\0' ) ? netname : network_name );

   if (!strcasecmp(server->network, "Unknown"))
{
   imc_shutdown( FALSE, server );
         imclog( "Cannot connect to %s as the netowrk name is unknown. Set the network name manually in imc.servers", server->servername );
   return;
}
   server->imcconnect_attempts = 0;
   imc_request_keepalive( server  );
   imc_firstrefresh( server );
}

/* Handle an autosetup response from a supporting server - Samson 8-12-03 */
void imc_handle_autosetup( char *source, char *servername, char *cmd, char *txt, char *encrypt, SERVER_DATA *server)
{
   if( !strcasecmp( cmd, "reject" ) )
   {
      if( !strcasecmp( txt, "connected" ) )
      {
         imclog( "There is already a mud named %s connected to the %s network.", this_imcmud->localname, server->network );
         imc_shutdown( FALSE, server );
         return;
      }
   ...
   if( !strcasecmp( cmd, "accept" ) )
   {
      ...
      if( encrypt && encrypt[0] != '\0' && !strcasecmp( encrypt, "SHA256-SET" ) )
      {
         ...
         server->sha256pass = TRUE;
         imc_save_config(  );
      }
      imc_finalize_connection( servername, txt, server );
      return;
   }

   imclog( "%s: Invalid autosetup response.", servername );
   imclog( "Data received: %s %s %s %s %s %s", source, servername, cmd, txt, encrypt, server->network );
   imc_shutdown( FALSE, server );
}

bool imc_write_socket( SERVER_DATA *server )
{
   ...
   while( nleft > 0 )
   {
      if( ( nwritten = send( server->desc, ptr, nleft, 0 ) ) <= 0 )
      {
         if( nwritten == -1 && errno == EAGAIN )
         {
            char *p2 = server->outbuf;
            ...
            *p2 = '\0';
            ...
            server->outtop = strlen( server->outbuf );
            return TRUE;
         }
   ...
   if( imcpacketdebug )
   {
      imclog( "Packet Sent: %s", server->outbuf );
      imclog( "Bytes sent: %d", server->outtop );
   }
   server->outbuf[0] = '\0';
   server->outtop = 0;
   return 1;
...
void imc_process_authentication( char *packet, SERVER_DATA *server )
{
   ...
   {
      imclog( "%s SHA-256 Authentication completed.", server->network );
      imc_finalize_connection( rname, pw, server );
      return;
   }
   ...
   {
      if( strcasecmp( server->serverpw, pw ) )
      {
      ...
      imc_finalize_connection( rname, netname, server );
      return;
   }
   ...
   {
      imc_handle_autosetup( command, rname, pw, version, netname, server );
      return;
   }
 ...
 */
bool imc_read_buffer( SERVER_DATA *server )
{
   size_t i = 0, j = 0;
   ...
   k = strlen( server->incomm );

   if( k < 0 )
      k = 0;

   for( i = 0; server->inbuf[i] != '\0'
        && server->inbuf[i] != '\n' && server->inbuf[i] != '\r' && i < IMC_BUFF_SIZE; i++ )
   {
      server->incomm[k++] = server->inbuf[i];
   }

   while( server->inbuf[i] == '\n' || server->inbuf[i] == '\r' )
   {
      ended = 1;
      i++;
   }
   ...
   while( ( server->inbuf[j] = server->inbuf[i + j] ) != '\0' )
      j++;
...
bool imc_read_socket( SERVER_DATA *server )
{
   size_t iStart, iErr;
   short loop_count = 0;
   bool begin = TRUE;

   iStart = strlen( server->inbuf );
   ...
   for( ;; )
   {
      ...
      nRead = recv( server->desc, server->inbuf + iStart, sizeof( server->inbuf ) - 10 - iStart, 0 );
      iErr = errno;
      if( nRead > 0 )
      {
         iStart += nRead;

         if( iStart >= sizeof( server->inbuf ) - 10 )
            break;

         begin = FALSE;
      }
      else if( nRead == 0 && server->state == IMC_ONLINE )
      {
         if( !begin )
            break;
      ...
      {
         imclog( "%s: Descriptor error on #%d: %s", __FUNCTION__, server->desc, strerror( iErr ) );
         return FALSE;
      }
   }
   server->inbuf[iStart] = '\0';
   return TRUE;
}
...
void imc_loop( void )
{
   ...
   if( server->imcwait == 1 )
   {
      if( ++server->imcconnect_attempts > 3 )
      {
         if( server->sha256pass )
         {
            imclog( "%s", "Unable to reconnect using SHA-256, trying standard authentication." );
            server->sha256pass = FALSE;
            imc_save_config( );
            server->imcconnect_attempts = 0;
         }
         else
         {
            server->imcwait = -2;
            imclog( "Unable to reestablish connection to %s:%d. Abandoning reconnect.", server->rhost, server->rport );
            continue;
         }
      }
   ...
   if( server->state == IMC_OFFLINE || server->desc == -1 )
      continue;
   ...
   FD_ZERO( &in_set );
   FD_ZERO( &out_set );
   FD_SET( server->desc, &in_set );
   FD_SET( server->desc, &out_set );

   null_time.tv_sec = null_time.tv_usec = 0;

   if( select( server->desc + 1, &in_set, &out_set, NULL, &null_time ) < 0 )
   {
   ...
   if( FD_ISSET( server->desc, &in_set ) )
   {
      if( !imc_read_socket( server ) )
      {
         if( server->inbuf && server->inbuf[0] != '\0' )
         {
            if( imc_read_buffer( server ) )
            {
               if( !strcasecmp( server->incomm, "SHA-256 authentication is required." ) )
               {
                  ...
                  imclog( "Buffer contents: %s", server->incomm );
            }
         }
         FD_CLR( server->desc, &out_set );
         imc_shutdown( TRUE, server );
         continue;
      }

      while( imc_read_buffer( server ) )
      {
         if( imcpacketdebug )
            imclog( "Packet received: %s", server->incomm );
         ...
         switch ( server->state )
         {
            ...
            case IMC_AUTH2:  /* Now you've contacted the server and need to process the authentication response */
               imc_process_authentication( server->incomm, server );
               server->incomm[0] = '\0';
               break;

            case IMC_ONLINE: /* You're up, pass the bastard off to the packet parser */
               imc_parse_packet( server->incomm );
               server->incomm[0] = '\0';
               break;
         }
      }
   ...
   if( server->desc > 0 && server->outtop > 0 && FD_ISSET( server->desc, &out_set ) && !imc_write_socket( server ) )
   {
      server->outtop = 0;
      imc_shutdown( TRUE, server );
      }
...
bool imc_is_connected (void)
{
	SERVER_DATA *server;
      ...
      for( server = first_server; server; server = server->next ) {
		  if (!server)
            continue;
             if (server->state == IMC_ONLINE)
				 return (TRUE);
             }
			 return(FALSE);
}

bool imc_is_connecting (void)
{
	SERVER_DATA *server;
...
bool imc_loadchar( CHAR_DATA * ch, FILE * fp, const char *word )
{
   ...
   {
      case 'I':
		 imclog( "imc_loadchar word %s", word);
         ...
         if( !strcasecmp( word, "IMClisten" ) )
         {
            ...
            if( IMC_LISTEN( ch ) != NULL && imc_is_connected() == TRUE )
            {
               IMC_CHANNEL *channel = NULL;
               char *channels = IMCSTRALLOC(IMC_LISTEN( ch ));
               char arg[SMST];
         ...
         if( !strcasecmp( word, "IMCdeny" ) )
         {
            ...
            if( IMC_DENY( ch ) != NULL && imc_is_connected() == TRUE)
            {
               IMC_CHANNEL *channel = NULL;
...
#if defined(_DISKIO_H_) || (_TBAMUD)
/* This is used only by CircleMUDs which have the ASCII Pfile code installed */
void imc_load_pfile( CHAR_DATA * ch, const char *tag, char *line )
{

   if( !strcmp( tag, "IMCPrm" ) || !strcmp( tag, "IPrm" ) )
      IMCPERM( ch ) = atoi(line);
   if( !strcmp( tag, "IMCEml" ) || !strcmp( tag, "IEml" ) )
      IMC_EMAIL( ch ) = IMCSTRALLOC( line );
   if( !strcmp( tag, "IMCAIM" ) || !strcmp( tag, "IAIM" ) )
      IMC_AIM( ch ) = IMCSTRALLOC( line );
   if( !strcmp( tag, "IMCICQ" ) || !strcmp( tag, "IICQ" ) )
      IMC_ICQ( ch ) = atoi(line);
   if( !strcmp( tag, "IMCYah" ) || !strcmp( tag, "IYah" ) )
      IMC_YAHOO( ch ) = IMCSTRALLOC( line );
   if( !strcmp( tag, "IMCMSN" ) || !strcmp( tag, "IMSN" ) )
      IMC_MSN( ch ) = IMCSTRALLOC( line );
   if( !strcmp( tag, "IMCURL" ) || !strcmp( tag, "IURL" ) )
      IMC_HOMEPAGE( ch ) = IMCSTRALLOC( line );
   if( !strcmp( tag, "IMCCMT" ) || !strcmp( tag, "ICMT" ) )
      IMC_COMMENT( ch ) = IMCSTRALLOC( line );
   if( !strcmp( tag, "IMCFLG" ) || !strcmp( tag, "IFLG" ) ) {
      IMCFLAG( ch ) = atoi(line);
            imc_char_login( ch );
}
   if( !strcmp( tag, "IMCLSN" )|| !strcmp( tag, "ILSN" ) )
   {
   ...
   if( !strcmp( tag, "IMCDNY" ) || !strcmp( tag, "IDNY" ) )
   {
   ...
   if( !strcmp( tag, "IMCIGN" ) || !strcmp( tag, "IIGN" ) )
   {
      IMC_IGNORE *temp;
...
/* This is used only by CircleMUDs which have the ASCII Pfile code installed */
void imc_save_pfile( CHAR_DATA * ch, FILE * fp )
{
   IMC_IGNORE *temp;
...
void IMC_saveservers( void )
{
   ...
   {
      fprintf( fp, "%s",
               "#This server is expecting SHA256 authentication now. Do not remove this line unless told to do so.\n" );
...
void IMC_readserver( SERVER_DATA *server, FILE *fp )
{
   const char *word;
   bool fMatch;
   ...
   for ( ; ; )
   {
	...
	switch( word[0] )
	{
	   ...
	   case 'E':
         ...
         case 'C':
            IMCKEY( "ClientPwd", server->clientpw, imcfread_line( fp ) );
            break;

         case 'N':
            IMCKEY( "Network", server->network, imcfread_line( fp ) );
            break;

         case 'S':
            IMCKEY( "ServerPwd", server->serverpw, imcfread_line( fp ) );
            IMCKEY( "ServerAddr", server->rhost, imcfread_line( fp ) );
            IMCKEY( "ServerPort", server->rport, imcfread_number( fp ) );
            IMCKEY( "SHA256", server->sha256, imcfread_number( fp ) );
            IMCKEY( "SHA256Pwd", server->sha256pass, imcfread_number( fp ) );
            break;
	}
	if( !fMatch )
	   imcbug( "imc_readserver: no match: %s", word );
   }
}

bool load_servers( int desc )
{
   FILE *fp;
   SERVER_DATA *server;

   first_server = last_server = NULL;

   imclog( "%s", "Loading IMC server data..." );

   if( !( fp = fopen( IMC_SERVER_FILE, "r" ) ) )
   {
	imclog( "%s", "No server data found." );
	return FALSE;
   }
   ...
   for ( ; ; )
   {
	...
	{
	   IMCCREATE( server, SERVER_DATA, 1 );
         server->network = IMCSTRALLOC( "Unknown" );
         server->sha256 = TRUE;
         server->sha256pass = FALSE;
         server->desc = desc;
	   IMC_readserver( server, fp );
	   IMCLINK( server, first_server, last_server, next, prev );
	   continue;
	}
	else if( !strcasecmp( word, "END" ) )
	   break;
	...
	{
	   imcbug( "imc_load_servers: bad section: %s.", word );
	   continue;
	}
   }
   IMCFCLOSE( fp );
...
void imc_save_config( void )
{
   ...
   fprintf( fp, "ChannSep       %c\n\n", this_imcmud->channsep );
   fprintf( fp, "%s", "End\n" );
   fprintf( fp, "%s", "\n$END\n" );
   IMCFCLOSE( fp );
   IMC_saveservers(  );
...
void imcfread_config_file( FILE * fin )
{
   ...
   for( ;; )
   {
      ...
      switch ( word[0] )
      {
         ...
         case 'C':
		 if( !strcasecmp( word, "ChannSep" ) )
		 {
			 char *c;
			 c = imcfread_line(fin);
			 this_imcmud->channsep = *c;
			 IMCSTRFREE(c);
			 fMatch = TRUE;
			 break;
		 }
	    /* Add in case 'T': Tell Delim. */
              break;
...
int ipv4_connect( SERVER_DATA *server )
{
    ...
    */
   if( !inet_aton( server->rhost, &sa.sin_addr ) )
   {
      hostp = gethostbyname( server->rhost );
      if( !hostp )
      {
...
#else
   sa.sin_addr.s_addr = inet_addr(server->rhost);
#endif

   sa.sin_port = htons( server->rport );
...
bool imc_server_connect( SERVER_DATA *server )
{
#if defined(IPV6)
...
#else
   desc = ipv4_connect( server );
   if( desc < 1 )
      return FALSE;
#endif

   imclog( "Connecting to %s:%d.", server->rhost, server->rport );

   server->state = IMC_AUTH2;
   server->desc = desc;
   server->inbuf[0] = '\0';
   server->outsize = 1000;
   ...
   if( server->sha256 )
   {
      ...
      if( !server->sha256pass )
      {
         snprintf( buf, LGST, "PW %s %s version=%d autosetup %s SHA256",
                   this_imcmud->localname, server->clientpw, IMC_VERSION, server->serverpw );
      }
      /*
...
void free_imcdata( bool complete )
{
   ...
   for( server = first_server; server; server = next_server )
   {
      ...
      IMCSTRFREE( server->servername );
      IMCSTRFREE( server->rhost );
      IMCSTRFREE( server->network );
      IMCSTRFREE( server->serverpw );
      IMCSTRFREE( server->clientpw );
      IMCUNLINK( server, first_server, last_server, next, prev );
      IMCDISPOSE( server );
   }
   }
}
...
void imc_hotboot( void )
{
   ...
   if( this_imcmud && imc_is_connected() == TRUE )
   {
...
for( server = first_server; server; server = server->next )
 {
   if( server && server->state == IMC_ONLINE )
 {
         fprintf( fp, "%s %s\n", ( server->network ? server->network : "Unknown" ),
                  ( server->servername ? server->servername : "Unknown" ) );
 }
}
         IMCFCLOSE( fp );
         imc_savehistory(  );
...
/* Shutdown IMC2 */
void imc_shutdown( bool reconnect, SERVER_DATA *server )
{
   if( server && server->state == IMC_OFFLINE )
      return;

   imclog( "Shutting down %s network.", server->network );
...
if (imc_is_connected() == FALSE)
{
   ...
   if( reconnect )
   {
      server->imcwait = 100; /* About 20 seconds or so */
      imclog( "Connection to %s server was lost. Reconnecting in approximately 20 seconds.", server->network );
   } else {
      server->imcwait = -2;
      server->imcconnect_attempts = 4;
   }
}

/* Startup IMC2 */
bool imc_startup_network( bool connected, SERVER_DATA *serverc )
{
   ...
   if( connected )
   {
      ...
      else
      {
         ...
         IMCSTRFREE( serverc->network );
         serverc->network = IMCSTRALLOC( netname );
         IMCSTRFREE( serverc->servername );
         serverc->servername = IMCSTRALLOC( server );
         IMCFCLOSE( fp );
      }
      serverc->state = IMC_ONLINE;
      serverc->inbuf[0] = '\0';
      serverc->outsize = IMC_BUFF_SIZE;
      IMCCREATE( serverc->outbuf, char, serverc->outsize );
      imc_request_keepalive( serverc );
      imc_firstrefresh( serverc );
      return TRUE;
   }
...
SERVER_DATA *server, *serverc;
      for( server = first_server; server; server = server->next ) {
bool found = FALSE, online = FALSE;

if ( !( server ))
continue;

 if (server->state > IMC_OFFLINE || server->imcconnect_attempts > 3)
      continue;

 /* If we are allready connected or connecting to the network, lets not attemp another connection to another server on that same network */
      for( serverc = first_server; serverc; serverc = serverc->next ) {
		  if (server && serverc && server != serverc && !strcasecmp( server->network, serverc->network ) && serverc->state > IMC_OFFLINE)
		  {
			  found = TRUE;
          if (serverc->state == IMC_ONLINE)
           online = TRUE;
		  }
	  }
	  if (found) {
	  if (online)
		  continue;
  server->imcwait = 500; // give some time for first server to connect
          continue;
	  }

   if( ( !this_imcmud->autoconnect && !force && !connected ) || ( connected && server->desc < 1 ) )
   {
      imclog( "%s", "IMC2 network data loaded. Autoconnect not set. IMC2 will need to be connected manually." );
      return;
   }
   else
      imclog( "IMC2 %s network data loaded.", server->network );

   if( this_imcmud->autoconnect || force || connected)
   {
      if( imc_startup_network( connected, server ) )
      {
         imc_loadchannels(  );
         imc_loadhistory(  );
         imc_readbans(  );
...
IMC_CMD( imccommand )
{
   ...
   p = imc_newpacket( CH_IMCNAME( ch ), "ice-cmd", to , imc_network_of( imc_mudof(to) ));
...
IMC_CMD( imcsetup )
{
   ...
   if( imccmd[0] == '\0' || chan[0] == '\0' )
   {
      ...
      imc_to_char( "Where 'channel' is one of the following: to local name.\r\n", ch );
      for( c = first_imc_channel; c; c = c->next )
      {
      /*   if( c->local_name && c->local_name[0] != '\0' )
...
IMC_CMD( imctell )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      imc_printf(ch,  "Usage: imctell user%cmud <message>\n\r", this_imcmud->channsep );
   ...
   if( !check_mudof_delim( ch, buf, this_imcmud->channsep ) )
      return;
   ...
   if( argument[0] == '@' )
   {
      ...
      imc_send_tell( CH_IMCNAME( ch ), fix_sending( buf ), p, 2 );
...
IMC_CMD( imcfinger )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      if( !check_mudof_delim( ch, arg, this_imcmud->channsep ) )
         return;

      snprintf( name, LGST, "finger %s", imc_nameof( arg ) );
      imc_send_who( CH_IMCNAME( ch ), fix_sending(imc_mudof( arg )), name );
      return;
   }
...
IMC_CMD( imcbeep )
{
   ...
   if( !check_mudof_delim( ch, argument, this_imcmud->channsep ) )
      return;

   imc_send_beep( CH_IMCNAME( ch ), fix_sending(argument) );
...
IMC_CMD( imclist )
{
   ...
   if( argument && argument[0] != '\0' )
   {
    ...
    if (imc_is_connected() == TRUE)
    {
   imcpager_printf( ch, "~WActive muds :~!\r\n" );
   ...
   /* if we cannot do anything with them, why even list them?  Less spam this way. */
	  if ( !(p->expired )) {
      count++;
   ...
   imcpager_printf( ch, "\r\n~RRed ~Wmud names indicate connections that are down." ); */
   imcpager_printf( ch, "\r\n~W%d active muds found.\r\n", count);
	}
}
...
IMC_CMD( imcdisconnect )
{
   ...
   if( !*argument || argument[0] == '\0' )
   {
      imc_to_char( "~wWhich server number, or all? see 'imcconfig show'\r\n", ch );
      return;
   }
   ...
   imc_shutdown( FALSE, server );
      ...
      imc_to_char( "~wServer is either not found or not connected\r\n", ch );
}
...
IMC_CMD( imcconfig )
{
   ...
   argument = imcone_argument( argument, arg1 );
   argument = imcone_argument( argument, arg2 );

   if( arg1[0] == '\0' )
   {
      imc_to_char( "~wSyntax: ~Gimcconfig <field> [value]\r\n\r\n", ch );
      ...
      imc_to_char( "~wChannSep       : ~GName @ Mud Separator.\n\r", ch );
      imc_to_char( "~wAddserver <ip> : ~Gadd a new server.\r\n", ch );
      imc_to_char( "~wSyntax: ~Gimcconfig <server number> <field> [value]\r\n\r\n", ch );
      imc_to_char( "~w# ServerAddr     : ~GDNS or IP address of the server your mud connects to.\r\n", ch );
      imc_to_char( "~w# ServerPort     : ~GPort of the server your mud connects to.\r\n", ch );
      imc_to_char( "~w# Newtwork       : ~GServer network information. 'this should normally be set automatically'\r\n", ch );
      imc_to_char( "~w# ClientPwd      : ~GClient password for your mud to this server.\r\n", ch );
      imc_to_char( "~w# ServerPwd      : ~GServer password for your mud to this server.\r\n", ch );
      imc_to_char( "~w# deleteserver   : ~GDelete a server. (must be disconnected from that server)\r\n", ch );
      return;
   }
   ...
   count = 1;
      ...
      imc_printf( ch, "~wChannSep       : ~G%c\n\r", this_imcmud->channsep );

        for( server = first_server; server; server = server->next, count++ ) {
			if (server)
			{
         imc_to_char( "\r\n", ch );
      imc_printf( ch, "~wServer# %d ", count );
      if( server->state == IMC_ONLINE )
      ...
      if( server->sha256 && server->sha256pass )
   ...
   {
      IMCSTRFREE( this_imcmud->fullname );
      this_imcmud->fullname = IMCSTRALLOC( arg2 );
      imc_save_config(  );
   ...
   {
      IMCSTRFREE( this_imcmud->ihost );
      this_imcmud->ihost = IMCSTRALLOC( arg2 );
      imc_save_config(  );
   ...
   {
      IMCSTRFREE( this_imcmud->email );
      this_imcmud->email = IMCSTRALLOC( arg2 );
      imc_save_config(  );
   ...
   {
      IMCSTRFREE( this_imcmud->www );
      this_imcmud->www = IMCSTRALLOC( arg2 );
      imc_save_config(  );
   ...
   if( !strcasecmp( arg1, "infobase" ) )
   {
      ...
      IMCSTRFREE( this_imcmud->base );
      this_imcmud->base = IMCSTRALLOC( arg2 );
      imc_save_config(  );
   ...
   {
      IMCSTRFREE( this_imcmud->details );
      this_imcmud->details = IMCSTRALLOC( arg2 );
      imc_save_config(  );
      imc_to_char( "Infodetails updated.\r\n", ch );
      return;
   }

   if( !strcasecmp( arg1, "addserver" ) )
   {
	   IMCCREATE( server, SERVER_DATA, 1 );
         server->rhost = IMCSTRALLOC( arg2 );
         server->rport = atoi( "4000" );
         server->network = IMCSTRALLOC( "NotSet" );
         server->clientpw =  IMCSTRALLOC( "NotSet" );
         server->serverpw =  IMCSTRALLOC( "NotSet" );
         server->sha256 = FALSE;
         server->sha256pass = FALSE;
         server->desc = -1;
	   IMCLINK( server, first_server, last_server, next, prev );
   ...
   if( !strcasecmp( arg1, "channsep" ) )
   {
      this_imcmud->channsep = *arg2;
      imc_printf( ch, "ChannSep changed to %s\n\r", arg2 );
      imc_save_config(  );
      return;
   }
    ...
    count = 1;
     ...
     if (server && server->state > IMC_OFFLINE)
      ...
      imc_printf( ch, "Cannot alter %s while the mud is connected to %s.\r\n", arg1 , server->servername);
       return;
     }

   if( !strcasecmp( arg2, "deleteserver" ) )
   {
   IMCUNLINK( server, first_server, last_server, next, prev );
   IMCSTRFREE( server->servername );
   IMCSTRFREE( server->rhost );
   ...
   if( !*argument || argument[0] == '\0' )
	{
      imc_to_char( "Syntax imcconfig <server number> <field> [value]\r\n", ch );
   return;
}

  if( !strcasecmp( arg2, "serveraddr" ) )
   {
      IMCSTRFREE( server->rhost );
      server->rhost = IMCSTRALLOC( argument );
      imc_printf( ch, "ServerAddr changed to %s\r\n", argument );
      imc_save_config(  );
      return;
   }

   if( !strcasecmp( arg2, "serverport" ) )
   {
      server->rport = atoi( argument );
      imc_printf( ch, "ServerPort changed to %d\r\n", server->rport );
      imc_save_config(  );
      return;
   }

   if( !strcasecmp( arg2, "clientpwd" ) )
   {
      IMCSTRFREE( server->clientpw );
      server->clientpw = IMCSTRALLOC( argument );
      imc_printf( ch, "Clientpwd changed to %s\r\n", argument );
      imc_save_config(  );
      return;
   }

   if( !strcasecmp( arg2, "serverpwd" ) )
   {
      IMCSTRFREE( server->serverpw );
      server->serverpw = IMCSTRALLOC( argument );
      imc_printf( ch, "Serverpwd changed to %s\r\n", argument );
      imc_save_config(  );
      return;
   }

   if( !strcasecmp( arg2, "network" ) )
   {
      IMCSTRFREE( server->network );
      server->network = IMCSTRALLOC( argument );
      imc_printf( ch, "Network changed to %s\r\n", argument );
      imc_save_config(  );
      return;
    }
  }
...
IMC_CMD( imcpermset )
{
   ...
   {
      IMC_CHANNEL *channel = NULL;
      char *channels = IMCSTRALLOC(IMC_LISTEN( victim ));
...
IMC_CMD( imcchanwho )
{
   ...
   if( strcasecmp( argument, "all" ) )
   {
      while( argument[0] != '\0' )
      {
         ...
         p = imc_newpacket( CH_IMCNAME( ch ), "ice-chan-who", mud , imc_network_of( mud ));
      ...
      return;
        ...
        for( server = first_server; server; server = server->next ) {
			if (server && server->state == IMC_ONLINE)
			{
   p = imc_newpacket( CH_IMCNAME( ch ), "ice-chan-who", "*" , server->network);
   imc_addtopacket( p, "level=%d", IMCPERM( ch ) );
   imc_addtopacket( p, "channel=%s", c->name );
   imc_addtopacket( p, "lname=%s", c->local_name ? c->local_name : c->name );
   imc_write_packet( p );
...
IMC_CMD( imcremoteadmin )
{
   ...
   p = imc_newpacket( CH_IMCNAME( ch ), "remote-admin", to , imc_network_of( imc_mudof(to) ));
   imc_addtopacket( p, "command=%s", cmd );
   if( argument && argument[0] != '\0' )
      imc_addtopacket( p, "data=%s", argument );
serverc = imc_find_network( p );
   if( serverc->sha256pass )
   {
      char cryptpw[LGST];
      char *hash;
...
IMC_CMD( imclast )
{
   ...
   IMC_PACKET *p;
        ...
        for( server = first_server; server; server = server->next ) {
			if (server && server->state == IMC_ONLINE)
			{
   p = imc_newpacket( CH_IMCNAME( ch ), "imc-laston", server->servername , server->network);
   if( argument && argument[0] != '\0' )
...
char *imc_act_string( const char *format, CHAR_DATA * ch, CHAR_DATA * vic )
{
   ...
   static const char *const him_her[] = { "it", "him", "her", "hir" };
   static const char *const his_her[] = { "its", "his", "her", "hir" };
   ...
   while( *format != '\0' )
   {
      ...
      else
      {
         switch ( *format )
         {
            ...
            case 'e':
               i = should_upper ?
                  imccapitalize( he_she[URANGE( 0, CH_IMCSEX( ch ), 3 )] ) : he_she[URANGE( 0, CH_IMCSEX( ch ), 3 )];
               break;

            case 'E':
               i = should_upper ?
                  imccapitalize( he_she[URANGE( 0, CH_IMCSEX( vic ), 3 )] ) : he_she[URANGE( 0, CH_IMCSEX( vic ), 3 )];
               break;

            case 'm':
               i = should_upper ?
                  imccapitalize( him_her[URANGE( 0, CH_IMCSEX( ch ), 3 )] ) : him_her[URANGE( 0, CH_IMCSEX( ch ), 3 )];
               break;

            case 'M':
               i = should_upper ?
                  imccapitalize( him_her[URANGE( 0, CH_IMCSEX( vic ), 3 )] ) : him_her[URANGE( 0, CH_IMCSEX( vic ), 3 )];
               break;

            case 's':
               i = should_upper ?
                  imccapitalize( his_her[URANGE( 0, CH_IMCSEX( ch ), 3 )] ) : his_her[URANGE( 0, CH_IMCSEX( ch ), 3 )];
               break;

            case 'S':
               i = should_upper ?
                  imccapitalize( his_her[URANGE( 0, CH_IMCSEX( vic ), 3 )] ) : his_her[URANGE( 0, CH_IMCSEX( vic ), 3 )];
               break;
...
const char *imc_send_social( CHAR_DATA * ch, char *argument, int telloption )
{
   ...
   if( argument && argument[0] != '\0' )
   {
      if( !( ps = strchr( argument, this_imcmud->channsep ) ) )
      {
         imc_printf( ch,  "You need to specify a person%cmud for a target.\r\n", this_imcmud->channsep );
         return "";
      }
      else
      {
         ...
         {
            person[x] = argument[x];
            if( person[x] == this_imcmud->channsep )
               break;
         }
         person[x] = '\0';
...
bool imc_command_hook( CHAR_DATA * ch, const char *command, char *argument )
{
   ...
   for( cmd = first_imc_command; cmd; cmd = cmd->next )
   {
      ...
      {
         if( cmd->connected == TRUE && imc_is_connected() == FALSE )
         {
            imc_to_char( "The mud is not currently connected to IMC2.\r\n", ch );
            return TRUE;
         }
