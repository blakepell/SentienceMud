...
void do_mpstat(CHAR_DATA *ch, char *argument)
{
	...
	if (!victim->pIndexData->progs)
		...
		for(i = 0, slot = 0; slot < TRIGSLOT_MAX; slot++) {
			for(mprg = victim->pIndexData->progs[slot]; mprg; mprg = mprg->next) {
...
SCRIPT_CMD(do_mpat)
{
	...
	// This used to check the char_list, but this is just plain faster!
...
SCRIPT_CMD(do_mpcall)
{
	...
	ret = execute_script(script->vnum, script, info->mob, NULL, NULL, NULL, ch, obj1, obj2, vch, NULL,info->phrase,info->trigger);
	if(info->mob) {
...
SCRIPT_CMD(do_mpecho)
{
	...
	if(!buf[0]) return;
	act(buf, info->mob, NULL, NULL, TO_ROOM);
}
...
SCRIPT_CMD(do_mpechonotvict)
{
	...
	act(buf, attacker, NULL, victim, TO_NOTVICT);
}
	...
	if(!str_cmp(buf,"integer") || !str_cmp(buf,"number")) {
		...
		case ENT_STRING:
			if(is_number(arg.d.str)) {
				...
				vch = get_char_world(info->mob,arg.d.str);
			break;
		case ENT_MOBILE:
			vch = arg.d.mob;
			break;
		...
		if(mobs) {
			...
			vch = script_get_char_list(mobs, info->mob, FALSE, vnum, str);
		}
		...
		if(mobs) vch = script_get_char_list(mobs, info->mob, TRUE, 0, str);
		variables_set_mobile(info->var,name,vch);
		...
		obj = script_get_obj_list(objs, info->mob, 2, vnum, str);
		if(vch) variables_set_object(info->var,name,obj);
		...
		obj = script_get_obj_list(objs, info->mob, 1, vnum, str);
		variables_set_object(info->var,name,obj);
...
//	else if(!str_cmp(field,"material"))	ptr = (int*)&mob->material;
	...
	case '/':
		if (!value) {
			bug("MpAlterMob - adjust called with operator / and value 0", 0);
			return;
		}
		*ptr /= value;
		break;
...
SCRIPT_CMD(do_mpvarseton)
{
	...
	if(!str_cmp(buf,"integer") || !str_cmp(buf,"number")) {
		...
		obj = script_get_obj_list(objs, info->mob, 2, vnum, str);
		if(vch) variables_set_object(vars,name,obj);
	} else if(!str_cmp(buf,"worn")) {
		switch(arg.type) {
		case ENT_NUMBER:
			vnum = arg.d.num;
